...
from model.bart_model import BartForConditionalGeneration, BartForSequenceClassification
from transformers import AutoTokenizer, get_linear_schedule_with_warmup
from data_util.pretrain_data_util import PretrainDataModule
from pytorch_lightning.callbacks import LearningRateMonitor, ModelCheckpoint


class LitModelForPretrain(pl.LightningModule):
    def __init__(self, learning_rate, model_name, total_steps, ofname, mode=None):
        super().__init__()
        ...
        if mode == 'classification':
            self.model = BartForSequenceClassification.from_pretrained(model_name, num_labels=2) # for cola
            # self.model = BartForSequenceClassification.from_pretrained(model_name, num_labels=3) # for nli
        elif mode == 'generation':
            self.model = BartForConditionalGeneration.from_pretrained(model_name)
            # self.generation_model.model.set_pretrain_mode()
...
def main(hparams):
    ...
    if hparams.mode == 'classification':
        checkpoint_callback = ModelCheckpoint(
            dirpath='lightning_logs/pretrain',
            filename='{epoch:02d}-{train_loss:.2f}-hrnn-classification-2class'
        )
    elif hparams.mode == 'generation':
        checkpoint_callback = ModelCheckpoint(
            dirpath='lightning_logs/pretrain',
            filename='{epoch:02d}-{train_loss:.2f}-hrnn-generation'
        )
    else:
        exit('Specify a mode, exiting...')

    if hparams.load_checkpoint:
        model = LitModelForPretrain.load_from_checkpoint(
                                            checkpoint_path='lightning_logs/pretrain/epoch=02-train_loss=0.02-hrnn-classification.ckpt',
                                            # checkpoint_path='lightning_logs/pretrain/epoch=02-train_loss=0.02-b32-lr5e-5-hrnn.ckpt',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", default="generation", type=str)
